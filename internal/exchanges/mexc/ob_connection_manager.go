...
func (cm *OrderBookConnectionManager) Run() {
	log.Printf("[MEXC-OB-CONN-MANAGER] Starte Manager fuer %s", cm.marketType)
	for {
		select {
		...
		case <-cm.stopCh:
			log.Printf("[MEXC-OB-CONN-MANAGER] Stoppe Manager fuer %s", cm.marketType)
			cm.stopAllShards()
			return
		}
	}
...
func (cm *OrderBookConnectionManager) addSubscription(symbol string, depth int, freq string) {
	...
	for _, shard := range cm.shards {
		if cm.shardLoad[shard] < symbolsPerShard {
			log.Printf("[MEXC-OB-CONN-MANAGER] Sende 'subscribe' fuer %s an existierenden Shard.", symbol)
			shard.commandCh <- ShardCommand{Action: "subscribe", Symbols: []string{symbol}, Depth: depth, Freq: freq}
	...
	log.Printf("[MEXC-OB-CONN-MANAGER] Erstelle neuen Shard fuer %s.", symbol)
	stopCh := make(chan struct{})
