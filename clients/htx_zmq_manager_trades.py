...
def main() -> int:
    parser = argparse.ArgumentParser(description="Subscribe to HTX native trades/orderbooks via zmq_manager")
    ...
    sock.setsockopt(zmq.IDENTITY, f"htx-native-{uuid.uuid4().hex}".encode())
    ...
    print(
        f"[CLIENT] broker={args.broker} exchange=htx_native symbol={args.symbol} "
