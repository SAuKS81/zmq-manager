...
def normalize_orderbook_obj(ob) -> dict:
    ...
    return {
        ...
        "exchange": ob.get("exchange", ob.get("e")),
    }
...
def build_request(symbol: str, encoding: str, data_type: str, depth: int, frequency: str) -> dict:
    req = {
...
def build_unsubscribe(symbol: str, data_type: str, depth: int, frequency: str) -> dict:
    req = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Subscribe to MEXC native trades/orderbooks via zmq_manager")
    ...
    sock.setsockopt(zmq.IDENTITY, f"mexc-native-{uuid.uuid4().hex}".encode())
    ...
    sock.send_json(build_request(args.symbol, args.encoding, args.data_type, args.depth, args.frequency))

    poller = zmq.Poller()
    poller.register(sock, zmq.POLLIN)

    print(
        f"[CLIENT] broker={args.broker} exchange=mexc_native symbol={args.symbol} "
        f"data_type={args.data_type} depth={args.depth} frequency={args.frequency} encoding={args.encoding}"
    )
    ...
    try:
        sock.send_json(build_unsubscribe(args.symbol, args.data_type, args.depth, args.frequency))
    except Exception:
        pass
    finally:
        sock.close(0)
