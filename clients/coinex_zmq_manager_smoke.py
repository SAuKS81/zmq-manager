...
def load_active_coinex_symbols(market_type: str, quote: str, limit: int | None) -> list[str]:
    exchange = ccxt.coinex({"options": {"defaultType": "swap" if market_type == "swap" else "spot"}})
...
def build_unsubscribe(symbols: list[str], market_type: str, data_type: str) -> dict:
    return {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Smoke client for CoinEx native trades/orderbooks via zmq_manager")
    ...
    symbols = load_active_coinex_symbols(args.market_type, args.quote.upper(), limit)
    if not symbols:
        print("[CLIENT] no CoinEx symbols found")
        return 1

    ctx = zmq.Context()
    sock = ctx.socket(zmq.DEALER)
    sock.setsockopt(zmq.IDENTITY, f"coinex-smoke-{uuid.uuid4().hex}".encode())
    ...
    print(
        f"[CLIENT] broker={args.broker} exchange=coinex_native "
    ...
    try:
        sock.send_json(build_unsubscribe(symbols, args.market_type, args.data_type))
    except Exception:
        pass
    finally:
        sock.close(0)
