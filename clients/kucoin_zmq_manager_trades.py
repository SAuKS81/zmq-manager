...
def build_request(symbol: str, encoding: str, data_type: str, depth: int) -> dict:
    req = {
...
def build_unsubscribe(symbol: str, data_type: str, depth: int) -> dict:
    req = {
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Subscribe to KuCoin native spot trades/orderbooks via zmq_manager")
    ...
    sock.setsockopt(zmq.IDENTITY, f"kucoin-native-{uuid.uuid4().hex}".encode())
    ...
    sock.send_json(build_request(args.symbol, args.encoding, args.data_type, args.depth))

    poller = zmq.Poller()
    poller.register(sock, zmq.POLLIN)

    print(
        f"[CLIENT] broker={args.broker} exchange=kucoin_native symbol={args.symbol} "
    ...
    try:
        sock.send_json(build_unsubscribe(args.symbol, args.data_type, args.depth))
    except Exception:
        pass
    finally:
        sock.close(0)
